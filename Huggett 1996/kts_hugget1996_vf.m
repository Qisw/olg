function [savingv, savingg, savingg_ordered] = kts_hugget1996_vf(params, r, w, b, BQ0)
       ...
       %% Finding the value function and the policy function for each age group
        ...
        savingv(:,:,I) = uc;                                                % v(I) = uc + BETA*s(I+1)*v(I+1). By the way, v(I+1) = 0. 
        ...
        for i=(I-1):-1:1;
            for j=1:NZ;
                ...
                c = adjc(c);
                uc = c.^(1-SIGMA)*(1-SIGMA)^(-1);
                star = uc + BETA*s(i+1)*repmat((pi(j,:)*savingv(:,:,i+1)'), [NA,1]);
                [aa, bb] = max(star');
                savingv(:, j, i) = aa';
