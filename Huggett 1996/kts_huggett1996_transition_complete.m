...
NP = 320;                                                                               % transition 기간 - 160기 후에는 새로운 steady state에 들어갈 것으로 예상
...
if ZETA == 1; 
   BQmat0 = zeros(NP,1);                                                    % 상속세율 ZETA == 1인 경우 BQ=0이 되므로 grid를 만들 수 없음
else 
    BQmat0 = (result0.BQ: (result1.BQ-result0.BQ)/(NP - 1): result1.BQ)';   % 상속세율 ZETA != 1인 경우 steady state를 선으로 연결하는 grid를 설정
end;

BQmat1 = zeros(size(BQmat0));
BQTAXmat = zeros(size(rgrid0));
wmat  = zeros(size(rgrid0));
bmat  = zeros(I, size(rgrid0, 1));

BQmatmat = zeros(size(BQmat0,1),100);                                       % 수렴여부를 관찰하기 위해 iteration 마다 계산된 BQmat을 저장
...
while (max(abs(rgrid0 - rgrid1)) > 0.0001);
    ...
    for t=1:NP-1;                                                           % 뒤에서 부터 역으로 value function과 policy function을 계산한다.
        ...
        % Value Function Iteration: 유산의 초기값은 임의로 설정. 향후 순방향에서 유산값을 구하여 다음 역방향에서 사용하여 수렴여부를 관찰
        [savingvmat(:,:,:,NP-t), savinggmat(:,:,:,NP-t), savingg_orderedmat(:,:,:,NP-t)] = kts_hugget1996_vf_transition(sim_params, rgrid0(NP-t), wmat(NP-t), bmat(:,NP-t), BQmat0(NP-t), savingvmat(:,:,:,NP-t+1));    % value function, policy function, ordered policy function
            
        % Monte Carlo Simulation for a cohort        
        [savingpopmat(:,:,:,NP-t)] = kts_hugget1996_simulation(sim_params, savingg_orderedmat(:,:,:,NP-t));
                
        fprintf('it = %d, t = %d, r = %1.6f, KS = %3.4f, KD = %3.4f, BQ = %2.4f\n', it, t, rgrid0(NP-t), KSmat(NP-t), KDmat(NP-t), BQmat0(NP-t));

     end;    
          
     BQmat1(1) = result0.BQ;
     BQTAXmat(1) = result0.BQTAX;
     KSmat(1) = result0.KS;
     ...
     plot(rgrid1, 'r:', 'LineWidth', 3); hold off;
     ...
     subplot(1,2,2);
     plot(BQmatmat(:,it)); hold on;
     if it == 1; 
        plot((result0.BQ: (result1.BQ-result0.BQ)/(NP - 1): result1.BQ));
     else
        plot(BQmatmat(:,it-1));         
     end;
     if ZETA == 1;
        plot(zeros(NP,1)); hold off;
     else
     plot((result0.BQ: (result1.BQ-result0.BQ)/(NP - 1): result1.BQ)); hold off;
     end;
     legend('BQmat1', 'BQmat0', 'Initial');
     set(gca, 'Fontsize', 20);
     axis tight;
     drawnow;
