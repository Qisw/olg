...
class cohort:
    ...
    def valueNpolicy(self, p):
        ...
        for h in range(self.hN):
            for i in range(self.aN):
                ...
                self.co[-1,h,i] = (cash+qr[-1]*(hh[h]+gamma))/(1+psi)
                self.ro[-1,h,i] = (cash*psi-qr[-1]*(hh[h]+gamma))/((1+psi)*qr[-1])
    ...
    def simulatelife(self, p, ainit=0, hinit=0):
        ...
        for y in range(T-1):    # y = 0, 1,..., 58
            ...
            for h1 in range(self.hN):
                ...
                if c1 <= 0:
                    ...
                    v1 = self.util(c1, r1+hh[self.hpath[y]]) + beta*sp[y]*self.vtilde[y+1][h1](self.apath[y+1])
                    if v1 >= v0:
        ...
        cash = self.budget(-1, self.hpath[-1], 0, self.apath[-1], 0, p)
        self.cpath[T-1] = (cash+qr[T-1]*(hh[self.hpath[T-1]]+gamma))/(1+psi)
        self.rpath[T-1] = (cash*psi-qr[T-1]*(hh[self.hpath[T-1]]+gamma))/((1+psi)*qr[T-1])
    ...
    def findv(self, y, h0, h1, a0, a1, p):
        ...
        if cash + qr[y]*(hh[h0]+gamma) > 0:
            ...
            value = self.util(co, ro+hh[h0]) + beta*sp[y]*self.vtilde[y+1][h1](a1)
        else:
...
def spath(e, g):
    title = 'psi=' + str(g.psi) + 'qh=' + str(e.qh[-1]) + 'qr=' + str(e.qr[-1]) \
                    ...
                    + str(g.ltv) + 'dti=' + str(g.dti) + 'D-C=%2.2f'%(e.Dratio[-1]) + '.png'
...
def main1(psi=0.1, qh=0.9879, qr=0.04145, ltv=0.6, dti=1.2, tcost=0.05):
    e = state(TG=1, k=4.2, ng=1, dng=0, W=45, R=30, Hs=60, qh=qh, qr=qr)
    e.printprices()
    g = cohort(W=45, R=30, psi=psi, beta=0.96, tcost=tcost, gamma=0.99, aL=-1.2, aH=1.2, aN=251, ltv=ltv, dti=dti)
    g.valueNpolicy(e.p)
    g.simulatelife(e.p)
    e.aggregate([g])
    # e.update()
