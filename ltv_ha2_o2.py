...
class cohort:
    ...
    def findvpath(self, p):
        ...
        for y in range(T-1):    # y = 0, 1,..., 58
            ...
            v0 = self.neg
            for h1 in range(self.hN):
                budget = self.budget(y-T, self.hpath[y], h1, self.apath[y], self.apath[y+1], p)
                # if y >= T-self.R:    # y = -2, -3, ..., -60
                    
                    # self.apath[y]*(1+(1-tr[y])*r[y]) + (hh[self.hpath[y]]-hh[h1])*qh[y] \
                    #             - hh[self.hpath[y]]*qh[y]*(hh[self.hpath[y]]!=hh[h1])*tcost \
                    #             + b[y] + Tr[y] - self.apath[y+1]
                # else:
                #     budget = self.apath[y]*(1+(1-tr[y])*r[y]) + (hh[self.hpath[y]]-hh[h1])*qh[y] \
                #                 - hh[self.hpath[y]]*qh[y]*(hh[self.hpath[y]]!=hh[h1])*tcost \
                #                 + Tr[y] - self.apath[y+1] + (1-tw[y]-tb[y])*w[y]*self.ef[y]
                c1 = (budget+qr[y]*(hh[self.hpath[y]]+gamma))/(1+psi)
                r1 = (budget*psi-qr[y]*(hh[self.hpath[y]]+gamma))/((1+psi)*qr[y])
                if c1 <= 0:
                    pass
                else:
        ...
        budget = (1+(1-tr[T-1])*r[T-1])*self.apath[T-1] + b[T-1] + Tr[T-1] + hh[self.hpath[T-1]]*qh[T-1]*(1-self.tcost)
        self.cpath[T-1] = (budget+qr[T-1]*(hh[self.hpath[T-1]]+gamma))/(1+psi)
        self.rpath[T-1] = (budget*psi-qr[T-1]*(hh[self.hpath[T-1]]+gamma))/((1+psi)*qh[T-1])
...
def main1(psi=0.1, qh=1.1, qr=0.05, ltv=0.6, dti=1.2, tcost=0.05):
    e = state(TG=1, ng=1, dng=0, W=45, R=30, Hs=10, qh=qh, qr=qr)
    e.printprices()
    g = cohort(W=45, R=30, psi=psi, beta=0.96, tcost=tcost, gamma=1, aL=-1.0, aH=1.0, aN=201, ltv=ltv, dti=dti)
    g.findvpath(e.p)
    e.aggregate([g])
    e.update()
    e.printprices()
