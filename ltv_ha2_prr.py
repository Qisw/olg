...
class cohort:
        ...
        for y in range(-2, -(T+1), -1):
            ...
            for h0 in range(self.hN):
                ...
                for h1 in range(self.hN):
                    p = Process(target=self.finda, args=(y, h0, h1, at, ct, rt, vt, casht, p))
                    p.start()
                    jobs.append(p)
    ...
    def finda(self, y, h0, h1, at, ct, rt, vt, casht, p):
    ...
    def findv(self, y, h0, h1, a0, a1, p):
        ...
        return v if co > 0 else float('nan')
    ...
    def util(self, c, s):
        ...
        return log(c) + self.psi*log(s+self.gamma) if (c>0 and s+self.gamma>0) else float('nan')
...
if __name__ == '__main__':
    ...
    e.printprices()
    g = cohort(W=45, R=30,psi=10,beta=0.96, tcost=0.02, gamma=1, aL=-0.0, aH=4.0, aN=501)
    g.findvpath(e.p)
    e.aggregate([g])
    e.update()
    e.printprices()